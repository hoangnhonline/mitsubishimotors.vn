...
class Cate extends Db {
    ...
    function insertCate() {
       $parent_id = (int) $_POST['parent_id'];
        ...
        $cate_alias = $this->changeTitle($cate_name_en);

        $sql = "INSERT INTO category VALUES(NULL,'$cate_name_cn','$cate_name_en','$cate_name_en','$cate_alias',$parent_id,'$cate_name_en','$cate_name_en','$cate_name_en',$update_time)";								
    ...
    function updateCate($cate_id) {
        ...
        $cate_alias = $this->changeTitle($cate_name_en);
         ...
         $sql = "UPDATE category
                SET cate_name_cn = '$cate_name_cn',cate_name_vi = '$cate_name_en',cate_alias = '$cate_alias',
                idTL = $parent_id,update_time = $update_time,					
                seo_title = '$cate_name_en',seo_description = '$cate_name_en',seo_keyword = '$cate_name_en'					
                WHERE cate_id = $cate_id ";
        mysql_query($sql) or die(mysql_error() . $sql);
    }
    function updateCateCha($idTL) {
        $parent_id = 0;

        $cate_name_cn = $this->processData($_POST['tl_name_cn']);
        $cate_name_en = $this->processData($_POST['tl_name_en']);
        $cate_name_vi = $this->processData($_POST['tl_name_vi']);
        ...
        $cate_alias = $this->changeTitle($cate_name_vi);
         ...
         $sql = "UPDATE theloai
                SET TenTL_cn = '$cate_name_cn',TenTL_vi = '$cate_name_vi',TenTL_en = '$cate_name_en',TenTL_KhongDau = '$cate_alias'                					
                WHERE idTL = $idTL ";
        mysql_query($sql) or die(mysql_error() . $sql);
    }
    ...
    function insertDanhMucCha() {       
       ...
       // $parent_id = 0;
        ...
        $tl_alias = $this->changeTitle($tl_name_vi);
