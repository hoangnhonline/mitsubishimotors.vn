    ...
    <table>
      <tr class="left">
        ...
        <td><select name='category_id' id="category_id">
            ...
            ?>
            <optgroup id="idTL" name="idTL" value="<?php echo $row_cha['idTL']?>" label="<?php echo $row_cha['TenTL_vi']?>">
                <?php
            $rs_con = $modelCate->getListCate($row_cha['idTL']);           
                if(mysql_num_rows($rs_con) > 0) {while($row_con = mysql_fetch_assoc($rs_con)){
            ?>
                <option  <?php echo ($row['category_id']==$row_con['cate_id']) ? "selected" : ""; ?> value="<?php echo $row_con['cate_id']?>"><?php echo $row_con['cate_name_vi']?></option>
            <?php }}else{ ?>
                <option  <?php echo ($row['category_id']==$row_cha['cate_id']) ? "selected" : ""; ?> value="<?php echo $row_cha['cate_id']?>"><?php echo $row_cha['cate_name_vi']?></option>
            <?php } ?>
              </optgroup>
            <?php } ?>
          </select></td>
      ...
      <tr class="left">
        <td>{gia}</td>
        <td><input type="text" name="price" id="price" style="width:200px;height:25px" value="<?php echo $row['price_small']; ?>" /></td>
      </tr>      
      <tr class="left">
        <td>{hinhanh}</td>
